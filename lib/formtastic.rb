...
module Formtastic #:nodoc:
  ...
  class SemanticFormBuilder < ActionView::Helpers::FormBuilder
    ...
    @@default_cancel_button_accesskey = nil 
    @@default_reset_button_accesskey = nil 
    @@pretty_buttons_with_images = true
    ...
    cattr_accessor :default_text_field_size, :all_fields_required_by_default, :include_blank_for_select_by_default,
                   ...
                   :default_commit_button_accesskey, :default_cancel_button_accesskey, :default_reset_button_accesskey,
                   :pretty_buttons_with_images
    ...
    #
    #   * /images/formtastic/formtastic/tick.png
    #   * /images/formtastic/formtastic/cross.png
    #   * /images/formtastic/formtastic/refresh.png
    #
    ...
    def button(kind, options = {})
      ...
      options[:button_html][:accesskey] ||= case kind
        when :commit then @@default_commit_button_accesskey
        when :cancel then @@default_cancel_button_accesskey
        when :reset  then @@default_reset_button_accesskey
      end
      options[:url] ||= :back if kind == :cancel
      options[:label] ||= begin 
        object_name = @object ? @object.class.human_name : @object_name.to_s.send(@@label_str_method)
        key = (@object ? (@object.new_record? ? :create : :update) : :submit) if kind == :commit
        key ||= kind
        text = (self.localized_string(key, text, :action, :model => object_name) || ::Formtastic::I18n.t(key, :model => object_name))
      end
    ...
    protected
      ...
      def pretty_button(kind, options)
        if (options[:image] != false && pretty_buttons_with_images)# false indicates no image at all
        ...
        text = [image_html, options[:label]].compact.join(" ")
